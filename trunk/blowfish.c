...
Modifications by Xavier.Leroy@inria.fr, 2005. (Marked "XL".)
- Speed improvements
- Endianness handling.
...
/* XL: turned F into a macro + endianness handling */
...
void Blowfish_Encrypt(BLOWFISH_CTX *ctx, u32 *xl, u32 *xr){
  ...
  *xr = Xr;
  ...
  /* XL: loop unrolling */
  Xl ^= ctx->P[0];
  Xr ^= F(ctx,Xl) ^ ctx->P[1];    Xl ^= F(ctx,Xr) ^ ctx->P[2];
  Xr ^= F(ctx,Xl) ^ ctx->P[3];    Xl ^= F(ctx,Xr) ^ ctx->P[4];
  Xr ^= F(ctx,Xl) ^ ctx->P[5];    Xl ^= F(ctx,Xr) ^ ctx->P[6];
  Xr ^= F(ctx,Xl) ^ ctx->P[7];    Xl ^= F(ctx,Xr) ^ ctx->P[8];
  Xr ^= F(ctx,Xl) ^ ctx->P[9];    Xl ^= F(ctx,Xr) ^ ctx->P[10];
  Xr ^= F(ctx,Xl) ^ ctx->P[11];   Xl ^= F(ctx,Xr) ^ ctx->P[12];
  Xr ^= F(ctx,Xl) ^ ctx->P[13];   Xl ^= F(ctx,Xr) ^ ctx->P[14];
  Xr ^= F(ctx,Xl) ^ ctx->P[15];   Xl ^= F(ctx,Xr) ^ ctx->P[16];
  Xr ^= ctx->P[17];
...
void Blowfish_Decrypt(BLOWFISH_CTX *ctx, u32 *xl, u32 *xr){
  ...
  *xr = Xr;
  ...
  /* XL: loop unrolling */
  Xl ^= ctx->P[17];
  Xr ^= F(ctx,Xl) ^ ctx->P[16];    Xl ^= F(ctx,Xr) ^ ctx->P[15];
  Xr ^= F(ctx,Xl) ^ ctx->P[14];    Xl ^= F(ctx,Xr) ^ ctx->P[13];
  Xr ^= F(ctx,Xl) ^ ctx->P[12];    Xl ^= F(ctx,Xr) ^ ctx->P[11];
  Xr ^= F(ctx,Xl) ^ ctx->P[10];    Xl ^= F(ctx,Xr) ^ ctx->P[9];
  Xr ^= F(ctx,Xl) ^ ctx->P[8];     Xl ^= F(ctx,Xr) ^ ctx->P[7];
  Xr ^= F(ctx,Xl) ^ ctx->P[6];     Xl ^= F(ctx,Xr) ^ ctx->P[5];
  Xr ^= F(ctx,Xl) ^ ctx->P[4];     Xl ^= F(ctx,Xr) ^ ctx->P[3];
  Xr ^= F(ctx,Xl) ^ ctx->P[2];     Xl ^= F(ctx,Xr) ^ ctx->P[1];
  Xr ^= ctx->P[0];
